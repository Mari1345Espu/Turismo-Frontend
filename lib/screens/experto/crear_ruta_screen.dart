...
class _CrearRutaScreenState extends State<CrearRutaScreen> {
  ...
  void _verificarAutenticacion() {
    if (!AuthService.estaAutenticado) {
      LoggerService.warning('Intento de crear ruta sin autenticación');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              const Text('No estás autenticado. Los cambios no se guardarán.'),
      ...
      setState(() {
        error = 'Debes iniciar sesión para crear rutas.';
      });
      return;
    }
    ...
    if (!AuthService.esExperto) {
      LoggerService.warning(
          'Usuario con rol ${AuthService.rol} intentando crear ruta');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Solo expertos pueden crear rutas. Tu rol es: ${AuthService.rol ?? "no definido"}'),
          backgroundColor: Colors.orange,
      ...
      setState(() {
        error =
            'Solo usuarios con rol "experto" pueden crear rutas. Tu rol actual es: ${AuthService.rol ?? "no definido"}';
      });
    }
  }
  ...
  Future<void> _cargarLugaresDisponibles() async {
    try {
      ...
      LoggerService.info('Cargando lugares para crear ruta');

      final lugares = await ApiService.getLugares();

      setState(() {
        lugaresDisponibles = List<Map<String, dynamic>>.from(lugares);
        cargandoLugares = false;
      });

      LoggerService.info(
          '${lugaresDisponibles.length} lugares disponibles para crear ruta');
    } catch (e) {
  ...
  Future<void> _crearRuta() async {
    ...
    try {
      ...
      LoggerService.info('Enviando datos de nueva ruta: $rutaData');

      // Enviar datos al servidor mediante el ApiService
      await ApiService.crearRuta(rutaData);
      ...
      setState(() {
        error = 'Error al crear ruta: ${e.toString()}';
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(
        title: 'Crear Ruta de Viaje',
        showDrawer: false,
      ),
      body: cargandoLugares
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          children: [
                            ...
                            TextFormField(
                              ...
                              decoration: const InputDecoration(
                                ...
                                border: OutlineInputBorder(),
                                hintText:
                                    'Describe brevemente esta ruta turística',
                              ),
                              maxLines: 3,
                              validator: (value) {
                                if (value == null || value.isEmpty) {
                    ...
                    // Botón guardar
                    ElevatedButton.icon(
                      onPressed: enviandoFormulario ? null : _crearRuta,
                      ...
                      label: Text(
                          enviandoFormulario ? 'Guardando...' : 'Crear Ruta'),
